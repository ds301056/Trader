...
def load_data(pair, time_d=1):
    ...
    start = parser.parse("2020-10-01T00:00:00Z")  # Parse the start date
...
def run_ema_macd(ic: InstrumentCollection):
    # Function to run the EMA-MACD strategy across different instruments
    results = []  # Initialize a list to store results
    our_curr = ['USD', 'GBP', 'JPY', 'NZD']  # Define a list of currencies
    for p1 in our_curr:
        for p2 in our_curr:
            pair = f"{p1}_{p2}"  # Create currency pairs
            if pair in ic.instruments_dict.keys():  # Check if pair is valid
